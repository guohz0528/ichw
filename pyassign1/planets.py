...
"""turtle,color,radius of planet,a (hypothetically ,T is in proportion to a),c,b/a,phi
"""
...
para = [Mercury, Venus, Earth, Mars, Jupiter, Saturn, Halley]    #most of the data is fake
...
for h in range(385):
    for k in range(100):    #cycle for 38500 times
        for i in range(7):
            para[i][6] = starrun(para[i][0], para[i][3], para[i][4], para[i][5], para[i][6])
